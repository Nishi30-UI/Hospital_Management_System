...
class hospital():
     ...
     def insertFun(self):
         ...
         if id and name and bGroup and desease and point and medics and addr:
            ...
            try:
                 self.dbFun()
                 query = f"insert into hospital_table (id,name,b_group,desease,h_point,medication,addr) values(%s,%s,%s,%s,%s,%s,%s)"
                 self.cur.execute(query,(id,name,bGroup,desease,point,medics,addr))
                 self.con.commit()
                 self.tabFun()
                 self.table.delete(*self.table.get_children())
                 self.cur.execute("select * from hospital_table where id=%s",id)
     ...
     def dbFun(self):
         self.con = pymysql.connect(host="localhost", user="root", passwd="Nishi3009", database="hospital")
     ...
     def medicsFun(self):
         ...
         if pId:
             try:
                 self.dbFun()
                 query = f"select * from hospital_table where id=%s"
     ...
     def addPoint(self):
         ...
         if pId:
             try:
                 self.dbFun()
                 query = f"select h_point from hospital_table where id=%s"
                 self.cur.execute(query,pId)
                 val = self.cur.fetchone()
 
                 newPoint = val[0]+point
                 query2 = f"update hospital_table set h_point=%s where id=%s"
                 self.cur.execute(query2,(newPoint,pId))
                 self.con.commit()
                 self.tabFun()
                 self.table.delete(*self.table.get_children())
                 self.cur.execute("select * from hospital_table where id=%s", pId)
     ...
     def disFun(self):
         ...
         try:
             self.dbFun()
             query = f"delete from hospital_table where id =%s"
             self.cur.execute(query,pId)
             self.con.commit()
